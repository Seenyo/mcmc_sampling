...
def initialize_parameters():
    ...
    st.session_state.num_of_chains = st.sidebar.number_input("Number of Independent Trials", 1, 10000000,
                                                             10000)
    st.session_state.num_of_iterations_for_each_chain = st.sidebar.number_input("Number of Iterations for Each Trial",
                                                                                1, 10000000, 10000)
...
def perform_calculations():
    ...
    for i in range(len(st.session_state.result_particles)):
        for j in range(len(st.session_state.result_particles[i])):
            ...
            dist_mean = dist_sum / (st.session_state.num_of_particles * (st.session_state.num_of_particles - 1) / 2)
            if dist_mean < st.session_state.min_distance:
                st.session_state.min_distance = dist_mean
    ...
    st.info(f'Minimum mean distance is: {st.session_state.min_distance}')
